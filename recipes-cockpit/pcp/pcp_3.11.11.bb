...
SYSTEMD_SERVICE:${PN} = "pmcd.service"
...
SUMMARY:${PN}-testsuite = "Performance Co-Pilot (PCP) test suite"
DESCRIPTION:${PN}-testsuite = "Quality assurance test suite for Performance Co-Pilot (PCP)"
RDEPENDS:${PN}-testsuite = "${PN}"
FILES:${PN}-testsuite = "/var/lib/pcp/testsuite"

# pcp-manager package
PACKAGES =+ "${PN}-manager"
SUMMARY:${PN}-manager =  "Performance Co-Pilot (PCP) manager daemon"
DESCRIPTION:${PN}-manager = "\
...
scripts."
RDEPENDS:${PN}-manager = "${PN}"
FILES:${PN}-manager = "${systemd_unitdir}/system/pmmgr.service \
                       ${datadir}/pcp/lib/pmmgr\
                       ${sysconfdir}/pcp/pmmgr"
SYSTEMD_SERVICE:${PN}-manager = "pmmgr.service"
